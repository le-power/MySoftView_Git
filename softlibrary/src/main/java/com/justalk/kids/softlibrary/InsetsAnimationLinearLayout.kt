...
class InsetsAnimationLinearLayout @JvmOverloads constructor(
    ...
    private var imeDissMissEnable = 0
    companion object{
        const val IME_STATE_INIT = 0
        const val IME_STATE_DISSMISS = 1
        const val IME_STATE_RESET = 2
    }
    ...
    override fun onNestedPreScroll(target: View, dx: Int, dy: Int, consumed: IntArray, type: Int) {
        translateTouchStateCallBack?.onNestedPreScroll()
    ...
    override fun onNestedScroll(
        ...
        translateTouchStateCallBack?.onNestedScroll()
        //底部view弹出的时候,不滑动
        ...
        if (dyUnconsumed > 0) {
            if (imeDissMissEnable == IME_STATE_INIT){
                imeDissMissEnable = IME_STATE_DISSMISS
            }
    ...
    override fun onNestedFling(
        ...
        translateTouchStateCallBack?.onNestedFling()
        //在底部view显示的时候,不处理滑动
        if (EmotionKeyboard.bottomContainerShowFlag && velocityY > 0){
            return true
        }

        //未注册的时候,不处理滑动
        if (!EmotionKeyboard.translationEnable){
            return true
        }
...
//            }
            ...
            when {
                (imeDissMissEnable == IME_STATE_DISSMISS) &&  velocityY > 0 && scrollImeOnScreenWhenNotVisible && !imeVisible -> {
                    // If the fling is in a upwards direction, and the IME is not visible,
                    // start an control request with an immediate fling
                    imeAnimController.startAndFling(this, velocityY)
                    // Indicate that we reacted to the fling
                    imeDissMissEnable = IME_STATE_RESET
                    return true
                }
                velocityY < 0 && scrollImeOffScreenWhenVisible && imeVisible -> {
                    // If the fling is in a downwards direction, and the IME is visible,
                    ...
                    imeAnimController.startAndFling(this, velocityY)
                    // Indicate that we reacted to the fling
                    imeDissMissEnable = IME_STATE_INIT
                    return true
                }
            }
        }
    ...
    override fun onStopNestedScroll(target: View, type: Int) {
        translateTouchStateCallBack?.onStopNestedScroll(target, type)
    ...
    fun setTouchCallBack(translateTouchStateCallBack: TranslateTouchStateCallBack){
        this.translateTouchStateCallBack = translateTouchStateCallBack
    }
}
