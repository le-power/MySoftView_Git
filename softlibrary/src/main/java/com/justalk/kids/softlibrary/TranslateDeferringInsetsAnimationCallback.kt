...
class TranslateDeferringInsetsAnimationCallback(
    ...
    private var imeHeight = 0f

    private var translateDeferringStateCallBack: TranslateDeferringStateCallBack? = null
    override fun onProgress(
        ...
        //获取软键盘高度
        if (imeHeight < abs(view.translationY)) {
            imeHeight = abs(view.translationY)
        }

        translateDeferringStateCallBack?.onProgress(insets, runningAnimations)
        return insets
    }
    ...
    override fun onEnd(animation: WindowInsetsAnimationCompat) {
        ...
        translateDeferringStateCallBack?.onEnd(animation)
        imeHeight = 0f
    }

    fun getImeHeight(): Int {
        return imeHeight.toInt()
    }
